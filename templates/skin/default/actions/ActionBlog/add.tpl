...
<script type="text/javascript">
        var info_types = {
          open: '{$aLang.blog_create_type_open_notice}',
          close: '{$aLang.blog_create_type_close_notice}',
          secret: '{$aLang.plugin.secretblog.blog_create_type_secret_notice}'
        };
        function loadBlogInfoType(type) {
          jQuery('#blog_type_note').html(info_types[type]);
        }
	jQuery(document).ready(function($){
          loadBlogInfoType($('#blog_type').val());
	});
</script>
...
<form method="post" enctype="multipart/form-data" class="wrapper-content">
	...
	<select name="blog_type" id="blog_type" class="input-width-200" onChange="loadBlogInfoType(jQuery(this).val());">
		...
		<option value="secret" {if $_aRequest.blog_type=='secret'}selected{/if}>{$aLang.plugin.secretblog.blog_create_type_secret}</option>
	</select>
